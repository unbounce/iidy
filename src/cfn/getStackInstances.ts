...
export async function getStackInstancesMain(argv: GenericCLIArguments): Promise<number> {
  ...
  for (const reservation of instances.Reservations || []) {
    for (const instance of reservation.Instances || []) {
      if (argv.short) {
        writeLine(instance.PublicDnsName ? instance.PublicDnsName : instance.PrivateIpAddress);
      }
      else {
        ...
        writeLine(sprintf('%-42s %-15s %s %-11s %s %s %s', instance.PublicDnsName, instance.PrivateIpAddress, instance.InstanceId, instance.InstanceType, state, placement, formatTimestamp(renderTimestamp(instance.LaunchTime as Date))));
      }
    }
  }
  writeLine(cli.blackBright(`https://console.aws.amazon.com/ec2/v2/home?region=${region}#Instances:tag:aws:cloudformation:stack-name=${StackName};sort=desc:launchTime`));
  return SUCCESS;
}
